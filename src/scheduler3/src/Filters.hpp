...
template <typename T>
void random_vector(std::vector<T> &vector) {
  std::random_device rd;   // obtain a random number from hardware
  std::mt19937 eng(rd());  // seed the generator
  std::uniform_int_distribution<> distr(0, 10000);

  for (auto &item : vector) item = distr(eng);
}

template <typename T>
void random_matrix(std::vector<std::vector<T>> &matrix) {
  std::random_device rd;   // obtain a random number from hardware
  std::mt19937 eng(rd());  // seed the generator
  std::uniform_int_distribution<> distr(0, 10000);

  for (auto &line : matrix)
    for (auto &item : line) item = distr(eng);
}

template <typename T>
void random_number(T &number) {
  std::random_device rd;   // obtain a random number from hardware
  std::mt19937 eng(rd());  // seed the generator
  std::uniform_int_distribution<> distr(0, 10000);
  number = distr(eng);
