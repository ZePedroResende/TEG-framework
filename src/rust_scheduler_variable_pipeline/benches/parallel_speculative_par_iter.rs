            ...
            if !(*size >= 512 && *vector_size >= 1200) {
                for thread in [2, 3, 6, 12, 24, 36, 48, 60, 72, 84].iter() {
                    ...
                    group.bench_with_input(
                        ...
                        |b, (s, v)| {
                            let mut d: Vec<
                                Arc<RwLock<rust_scheduler::parallel_speculative::data::Data>>,
                            > =
                                (0..*v)
                                    .into_par_iter()
                                    .map(|_| {
                                        Arc::new(RwLock::new(
                                            rust_scheduler::parallel_speculative::data::Data::new(
                                                1000, *s as i32, 1000,
                                            ),
                                        ))
                                    })
                                    .collect::<Vec<
                                        Arc<
                                            RwLock<
                                                rust_scheduler::parallel_speculative::data::Data,
                                            >,
                                        >,
                                    >>();
