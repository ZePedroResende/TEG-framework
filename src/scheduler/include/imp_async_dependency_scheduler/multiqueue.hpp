...
namespace imp_async_dependency_scheduler{
    ...
    public:
        ...
        DataFunction next_to_process() {
            ...
            //e preciso verificar se todas as queue estas vazias
            input_cv.wait(lck, [this] { return !this->all_queue_empty(this->execution_queue_vector); });
        ...
        DataOutput get_ouput() {
            ...
            output_cv.wait(lck, [this] { return !this->all_queue_empty(this->output_queue_vector); });
            std::set<int> insertion_order = data_queue.get_insertion_order();
        ...
        template<typename T>
        bool all_queue_empty(std::vector<std::queue<T>> queues) {
            bool is_empty = true;
