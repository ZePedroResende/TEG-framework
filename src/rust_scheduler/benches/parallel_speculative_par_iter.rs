...
fn parallel_speculative_par_iter_bench(
    ...
    let _a: Vec<_> = data
        .into_par_iter()
        .map(|d| rust_scheduler::parallel_speculative::scheduler::scheduler(d, true, n_threads))
        .collect();
}
                ...
                if !(*size >= 512 && *vector_size >= 1200) {
                    for thread in [2, 3, 6, 12, 24, 36, 48, 60, 72, 84].iter() {
                        ...
                        group.bench_with_input(
                            BenchmarkId::new("parallel_speculative_par_iter", parameter_string),
                            &(*size, *vector_size),
                            |b, (s, v)| {
                                ...
                                b.iter(|| black_box(parallel_speculative_par_iter_bench(&mut d, *thread)));
                            },
                            );
                    }
                }
